...
•	Câu hỏi phỏng vấn kiểm tra thủ công dành cho người mới
•	Câu hỏi phỏng vấn kiểm tra thủ công dành cho người có kinh nghiệm
•	Câu hỏi phỏng vấn thời gian thực về kiểm tra thủ công
...
2. Ưu điểm và nhược điểm của Kiểm thử thủ công là gì?
...
•	Không cần phải thay đổi toàn bộ mã để thực hiện những thay đổi nhỏ.
•	Nhận phản hồi giao diện người dùng chính xác.
•	Khả năng xử lý các tình huống sử dụng khó khăn một cách tốt hơn.
...
•	Thiết lập các chiến lược đảm bảo chất lượng và tổ chức thử nghiệm theo từng giai đoạn.
•	Tổ chức và tiến hành các cuộc họp đánh giá.
•	Thực hiện và phân tích các trường hợp thử nghiệm.
•	Phát hiện và sửa lỗi.
•	Theo dõi lỗi hệ thống và thảo luận với đồng nghiệp.
...
•	Kiểm thử đơn vị : Về cơ bản, đây là một cách kiểm thử các đoạn mã được tách biệt một cách hợp lý trong một hệ thống được gọi là các đơn vị. Chủ yếu, nó tập trung vào độ chính xác chức năng của mô-đun độc lập.
•	Kiểm thử tích hợp : Kiểm thử phần mềm ở cấp độ này bao gồm việc kết hợp và kiểm thử các đơn vị riêng lẻ để xem liệu chúng có hoạt động cùng nhau như bình thường hay không. Thử nghiệm này tập trung vào giao diện giữa các mô-đun.
•	Kiểm tra hệ thống : Nó bao gồm việc kiểm tra toàn bộ các thành phần của sản phẩm để đảm bảo đáp ứng các yêu cầu chung của sản phẩm. Các loại thử nghiệm hệ thống bao gồm thử nghiệm hồi quy, thử nghiệm khả năng sử dụng và thử nghiệm chức năng.
...
Sau đây là những kỹ năng kiểm tra thủ công quan trọng cần có: -Định hướng chi tiết và có thể báo cáo kết quả kiểm tra một cách chuyên nghiệp.
•	Một khả năng phân tích mạnh mẽ.
•	Có khả năng thực hiện kiểm tra kỹ thuật.
...
•	Yêu cầu : Phần mềm có thể hoạt động tốt nhưng sẽ không hữu ích cho khách hàng nếu nó không đáp ứng được nhu cầu của họ. Phạm vi yêu cầu cho biết có bao nhiêu yêu cầu được kiểm tra.
...
•	Tóm tắt thử nghiệm : Tóm tắt một dòng về trường hợp thử nghiệm.
•	Description : Mô tả chi tiết về test case.
•	Điều kiện tiên quyết hoặc điều kiện tiên quyết : Tập hợp các điều kiện cần tuân theo trước khi thực hiện các bước kiểm tra.
•	Các bước kiểm thử : Các bước chi tiết để thực hiện các trường hợp kiểm thử.
•	Test Data : Giá trị dữ liệu test được sử dụng trong test case.
...
•	Người thực hiện : Tên người thực hiện test case
13. Thử nghiệm Tích cực và Tiêu cực là gì?
•	Kiểm tra tích cực : Đây là một loại quy trình kiểm tra trong đó ứng dụng phần mềm được xác thực dựa trên các tập dữ liệu hợp lệ làm đầu vào. Nó chỉ đơn giản được sử dụng để kiểm tra xem ứng dụng có thực hiện những gì nó được yêu cầu hay không.
 
•	Kiểm thử tiêu cực : Đây là một loại quy trình kiểm thử trong đó ứng dụng phần mềm được xác thực dựa trên các tập dữ liệu không hợp lệ làm đầu vào. Nó chỉ đơn giản được sử dụng để kiểm tra xem hệ thống có hiển thị lỗi khi đáng lẽ phải thực hiện hay không. Trong việc thực hiện các trường hợp kiểm thử, kiểm thử âm tính được coi là một yếu tố rất quan trọng.
...
15. Giải thích Trình điều khiển Kiểm tra và Sơ kết Kiểm tra.
...
•	Sơ khai kiểm thử : Sơ khai kiểm thử được sử dụng theo phương pháp thử nghiệm từ trên xuống và cho phép kiểm tra các cấp mã cao hơn khi các cấp mã thấp hơn chưa được phát triển. Nó phục vụ như một 'chương trình được gọi' khi các chương trình con đang được phát triển.
...
16. Tầm quan trọng của Kiểm tra bản địa hóa là gì?
...
•	Tỷ lệ rò rỉ khuyết tật: Nó biểu thị tỷ lệ giữa tổng số sản phẩm bị loại bỏ tiềm năng trên tổng sản lượng tổng thể.
•	Tỷ lệ loại bỏ khuyết tật: Nó biểu thị tỷ lệ tổng số loại bỏ trên tổng sản lượng tổng thể.
...
•	Error : Lỗi xảy ra khi có lỗi lập trình trong code khiến chương trình không thể thực thi hoặc biên dịch.
•	Khiếm khuyết : Khiếm khuyết là bất kỳ biến thể nào giữa kết quả thực tế và kết quả mong đợi được xác định bởi người kiểm tra hoặc nhà phát triển. Các lỗi thường được phát hiện sau khi sản phẩm được đưa vào sản xuất và chỉ được giải quyết trong giai đoạn phát triển.
•	Lỗi : Một lỗi phần mềm được phát hiện trong giai đoạn thử nghiệm là lỗi hoặc không khớp. Điều này ảnh hưởng đến chức năng và hiệu suất của phần mềm.
...
•	Lập kế hoạch kiểm thử : Giai đoạn này liên quan đến việc xác định chiến lược kiểm thử. Bước này xác định mục tiêu và phạm vi của dự án.
•	Phát triển trường hợp thử nghiệm : Giai đoạn này liên quan đến việc xác định và phát triển các trường hợp thử nghiệm chi tiết. Dữ liệu thử nghiệm cũng được nhóm thử nghiệm chuẩn bị để thử nghiệm.
•	Thiết lập môi trường thử nghiệm : Đây là thiết lập phần mềm và phần cứng mà các nhóm thử nghiệm sử dụng để thực hiện các trường hợp thử nghiệm.
•	Thực thi kiểm tra : Điều này liên quan đến việc thực thi mã và so sánh nó với kết quả mong đợi.
•	Kết thúc chu trình kiểm thử : Điều này liên quan đến việc triệu tập một cuộc họp của các thành viên trong nhóm kiểm thử để đánh giá và đánh giá các tiêu chí hoàn thành chu trình dựa trên chi phí, thời gian, phạm vi kiểm thử, chất lượng cũng như các mục tiêu và phần mềm kinh doanh quan trọng.
...
7. Bạn có ý nghĩa gì khi kiểm tra luồng dữ liệu?
...
•	Loại bỏ hoặc loại bỏ các biến không bao giờ được sử dụng sau khi được khai báo.
•	Xác định các biến được sử dụng nhưng không bao giờ được khai báo.
•	Phân bổ biến trước khi nó được sử dụng.
•	Xác định các biến được xác định nhiều lần trước khi sử dụng
8. Thuật ngữ 'chất lượng' có nghĩa là gì khi thử nghiệm?
...
•	Test Case : Đúng như tên gọi, test case là một tài liệu chứa dữ liệu test, kết quả mong đợi, điều kiện tiên quyết và điều kiện hậu. Mục đích của tài liệu này là đảm bảo sản phẩm phần mềm đáp ứng các yêu cầu cụ thể cho một kịch bản thử nghiệm cụ thể. Kiểm thử thủ công bao gồm việc người kiểm thử thực hiện các trường hợp kiểm thử một cách thủ công mà không cần dựa vào các công cụ tự động. Trong quá trình phát triển các trường hợp thử nghiệm, có thể xác định được các lỗ hổng trong thông số kỹ thuật.
•	Ma trận truy xuất nguồn gốc : Đây là một tài liệu, thường được chứa trong một bảng biểu mẫu, minh họa mối quan hệ giữa các yêu cầu và các thành phần khác của dự án từ đầu đến cuối. Nói một cách đơn giản, nó ánh xạ các yêu cầu của khách hàng tới các trường hợp thử nghiệm.
12. Giải thích RTM (Ma trận truy xuất nguồn gốc yêu cầu).
RTM (Ma trận truy xuất nguồn gốc yêu cầu) được định nghĩa là một công cụ được sử dụng để xác định và theo dõi các yêu cầu cũng như sản phẩm bàn giao của một dự án. Điều này được thực hiện bằng cách thiết lập một luồng cho từng thành phần. Ngoài ra, nó quản lý các yêu cầu tổng thể của dự án. Không có gì phức tạp về phương pháp này và bất cứ ai cũng có thể làm được.
...
13. Tầm quan trọng của thử nghiệm linh hoạt là gì?
...
•	Đảm bảo sản phẩm có chất lượng cao đáp ứng được sự mong đợi của khách hàng.
•	Giảm chi phí và tiết kiệm thời gian.
•	Có khả năng thích ứng cao và linh hoạt.
•	Hỗ trợ các nhà phát triển phát hành phần mềm càng sớm càng tốt và cải thiện chất lượng sản phẩm.
14. Sự khác biệt giữa hồi quy và kiểm tra lại là gì?
•	Kiểm thử hồi quy : Kiểm thử hồi quy hay còn gọi là kiểm thử chung, xoay quanh việc chạy lại các kiểm thử chức năng và phi chức năng. Nó đặc biệt được thực hiện để đảm bảo liệu phần mềm được phát triển và thử nghiệm trước đó có còn hoạt động như cũ sau khi thay đổi hay không. Nó có thể được thực hiện bằng tay hoặc sử dụng các bài kiểm tra tự động
•	Kiểm tra lại : Kiểm tra lại, còn được gọi là kiểm tra theo kế hoạch, được sử dụng cho các lỗi cụ thể sau khi được nhà phát triển sửa lỗi. Việc kiểm tra lại được thực hiện để kiểm tra tình huống trong cùng điều kiện môi trường sau khi phát hiện đã được khắc phục.
15. Kiểm thử hệ thống và Kiểm thử đơn vị là gì? Viết sự khác biệt giữa chúng.
...
•	Kiểm thử đơn vị : Trong kiểm thử đơn vị, mỗi thành phần của phần mềm được kiểm thử riêng lẻ. Nói chung, kiểm thử đơn vị được thực hiện bởi các nhà phát triển. Những hệ thống có nhiều sự phụ thuộc lẫn nhau giữa các mô-đun của chúng không thể được kiểm thử bằng thử nghiệm đơn vị.
16. Các loại Kiểm thử Tích hợp là gì?
Kiểm thử tích hợp bao gồm các loại sau:
•	Thử nghiệm vụ nổ lớn : Nó liên quan đến việc tích hợp tất cả các mô-đun và thành phần cùng một lúc và sau đó thử nghiệm chúng dưới dạng toàn bộ (đơn vị duy nhất). Khi kiểm tra các thành phần này cùng nhau, chúng được coi là một thực thể. Quá trình tích hợp sẽ không được tiến hành nếu tất cả các thành phần của đơn vị không được hoàn thiện.
•	Thử nghiệm từ dưới lên : Chiến lược này liên quan đến việc thử nghiệm các mô-đun cấp thấp hơn trước, sau đó chuyển sang các mô-đun cấp cao hơn. Miễn là các mô-đun cấp cao nhất đã được thử nghiệm thì quá trình này vẫn tiếp tục. Sau khi tích hợp và thử nghiệm các mô-đun cấp thấp hơn, cấp độ mô-đun tiếp theo sẽ được tạo.
 
•	Kiểm tra từ trên xuống : Chiến lược này bao gồm việc kiểm tra hệ thống phần mềm từ trên xuống dưới theo luồng điều khiển. Thử nghiệm được tiến hành đầu tiên trên các mô-đun cấp cao hơn, sau đó là thử nghiệm và tích hợp các mô-đun cấp thấp hơn để xác minh chức năng của phần mềm. Việc kiểm tra được thực hiện bằng cách sử dụng sơ khai khi một số mô-đun chưa sẵn sàng
...
18. Khai thác thử nghiệm và kết thúc thử nghiệm là gì?
...
•	Công cụ thực thi thử nghiệm
•	Kho lưu trữ tập lệnh thử nghiệm
...
•	Đã mở lại : QA mở lại/kích hoạt lại lỗi khi lỗi chưa được khắc phục.
•	Trì hoãn : Một lỗi không thể giải quyết được trong chu kỳ hiện tại sẽ được hoãn lại để phát hành trong tương lai.
•	Bị từ chối : Có ba lý do phổ biến để từ chối một lỗi: trùng lặp, không phải lỗi và không thể tái tạo.
20. Giải thích các kỹ thuật kiểm tra dựa trên kinh nghiệm.
...
•	Kiểm tra khói : Đây là một loại thử nghiệm được thực hiện để đảm bảo rằng các chức năng cấp tính của chương trình đang hoạt động tốt. Nó hoạt động như một sự xác nhận về việc liệu nhóm đảm bảo chất lượng có thể tiếp tục tiến hành thử nghiệm hay không.
...
22. Nghịch lý thuốc trừ sâu có ý nghĩa gì? Bạn có thể làm gì để vượt qua nó.
...
•	Tạo một bộ trường hợp thử nghiệm mới cho các thành phần khác nhau của phần mềm.
•	Thêm các trường hợp thử nghiệm mới vào các trường hợp thử nghiệm hiện có.
Việc sử dụng các phương pháp này có thể dẫn đến việc tìm ra nhiều lỗi hơn ở những khu vực mà mức độ lỗi đã giảm. Câu hỏi phỏng vấn thời gian thực về kiểm tra thủ công (Dựa trên kịch bản)
...
•	Khi các trường hợp thử nghiệm cần được chạy trong một khoảng thời gian ngắn (một hoặc hai lần).
•	Khi một người cần thực hiện thử nghiệm thăm dò, thử nghiệm khả năng sử dụng hoặc thử nghiệm đặc biệt.
...
•	Bất cứ khi nào các trường hợp thử nghiệm phải được chạy lặp đi lặp lại trong một khoảng thời gian dài.
•	Khi cần thực hiện kiểm tra hiệu suất, kiểm tra tải hoặc kiểm tra hồi quy.
•	Bất cứ khi nào một người muốn ghi lại quá trình thử nghiệm.
...
•	Khi các bài kiểm tra liên quan đến các bước lặp đi lặp lại.
•	Khi có nhiều triển khai nhanh chóng cho sản phẩm, hướng dẫn sử dụng sẽ trở nên rất tốn thời gian và dư thừa.
2. Bạn sẽ xác định thời điểm ngừng thử nghiệm bằng cách nào?
...
•	Nếu thời hạn được đáp ứng (thời hạn phát hành, thời hạn kiểm tra, v.v.) và không còn vấn đề nào có mức độ ưu tiên cao trong hệ thống.
•	Hoàn thành các trường hợp thử nghiệm với tỷ lệ đậu nhất định.
•	Ngay khi ngân sách thử nghiệm cạn kiệt.
•	Thời gian trung bình giữa hai lần thất bại cố hữu được gọi là MTBF (Thời gian trung bình giữa các lần thất bại). Khi MTBF khá cao, giai đoạn thử nghiệm có thể bị dừng tùy theo quyết định của các bên liên quan.
•	Ngay khi phạm vi bao phủ mã tự động đáp ứng giá trị ngưỡng được chỉ định và không có lỗi nghiêm trọng nào.
•	Nếu tỷ lệ lỗi giảm xuống dưới một mức nhất định.
•	Sau khi giai đoạn thử nghiệm Beta hoặc Alpha kết thúc.
...
•	Phát triển một chiến lược thử nghiệm hiệu quả.
...
•	Hãy đảm bảo rằng bạn không cố gắng kiểm tra tất cả các trường hợp cùng một lúc mà hãy ứng biến chúng khi thực hiện.
...
•	Quản lý chu kỳ phát hành ổn định bằng cách sử dụng công cụ quản lý kiểm tra.
...
7. Khi các yêu cầu vẫn còn thay đổi, cách tốt nhất để thử nghiệm sản phẩm là gì?
...
•	Giá trị đầu vào phía trên các cạnh cực trị: 2 và 21.
•	Giá trị đầu vào bên dưới các cạnh cực trị: 0 và 19.
Do đó, các giá trị biên là 0, 1, 2 và 19, 20, 21.
9. Làm sao bạn biết mã đã đáp ứng các thông số kỹ thuật?
